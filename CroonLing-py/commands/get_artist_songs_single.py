...
class GetSpotifySinglesCommands:
    ...
    def register(self):
        ...
        async def spotify_singles(ctx, *, artist_name: str):
            """Spotify API를 통해 특정 가수의 싱글 곡 목록을 가져옵니다."""
            await ctx.send(f"'{artist_name}'의 싱글 곡 정보를 불러오는 중입니다. 잠시만 기다려주세요...")
            ...
            try:
                ...
                for single in singles:
                    album_tracks_result = await self.spotify_api.get(f"albums/{single['id']}/tracks")
                ...
                if not track_list:
                    await ctx.send(f"'{artist_name}'의 싱글 곡 목록을 찾을 수 없습니다.")
                    return
                ...
                async def save_callback(interaction):
                    self.save_handler.save_artist_and_songs(
                        artist=artist_name,
                        album="Singles",  # 싱글 트랙을 모아둔 앨범 이름
