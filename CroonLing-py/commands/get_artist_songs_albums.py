    ...
    def register(self):
        ...
        async def spotify_artist_albums(ctx, *, artist_name: str):
            ...
            try:
                ...
                track_description = "\n".join(
                    [f"{idx + 1}. {song['song_title']}" for idx, song in enumerate(song_list)]
                )
                embed = discord.Embed(
                ...
                # 버튼 생성
                view = View()
                view.add_item(SaveButton(self.save_handler, selected_track))
                view.add_item(InfoButton(self.info_handler, selected_track))
                view.add_item(LyricsButton(self.lyrics_handler, selected_track))
                view.add_item(TranslationButton(self.translation_handler, self.lyrics_handler, self.save_handler, selected_track))
                view.add_item(PhoneticsButton(self.phonetics_handler, selected_track))
                view.add_item(KoreanPhoneticsButton(self.korean_phonetics_handler, selected_track))
                # view.add_item(SongNameButton(self.song_name_handler, selected_track))

                # 재생 버튼은 URL 연결
                play_button = discord.ui.Button(label="재생", url=selected_track['url'], style=discord.ButtonStyle.link)
                view.add_item(play_button)

                await ctx.send(f"'{selected_track['song_title']}'을(를) 선택하셨습니다.", view=view)
